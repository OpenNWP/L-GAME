...
module gradient_operators
	...
	use grid_generator, only: t_scalar, t_vector, t_grid
	...
	subroutine grad_hor(scalar_field, result_field, grid)
		...
		type(t_vector), intent(inout) :: result_field        ! resulting vector field
		type(t_grid),   intent(in)    :: grid                ! the grid properties

		! calculating the x component of the gradient
		do ji = 1,nlins
			do jk = 1,ncols-1
				result_field=>x(ji,jk+1,:) = (scalar_field=>values(ji,jk+1,:) - scalar_field=>values(ji,jk,:))/grid=>dx(ji,:)
			enddo
		enddo
		...
		do ji = 1,nlins-1
			do jk = 1,ncols
				result_field=>y(ji+1,jk:) = (scalar_field=>values(ji,jk,:) - scalar_field=>values(ji+1,jk,:))/grid=>dy(:)
			enddo
		enddo
