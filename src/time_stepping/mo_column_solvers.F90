      ...
      do ji=1,ny
        do jk=1,nx
          ...
          *(grid%theta_v_bg(ji,jk,n_layers)+state_new_used%theta_v_pert(ji,jk,n_layers))
          ...
          diag%power_flux_density_sens_soil(ji,jk) = (grid%land_fraction(ji,jk)+grid%lake_fraction(ji,jk)) &
          ...
          *(t_gas_lowest_layer_new - state_new_used%temperature_soil(ji,jk,1)))/diag%scalar_flux_resistance(ji,jk)
          diag%power_flux_density_sens_sea(ji,jk) = (1._wp-grid%land_fraction(ji,jk)-grid%lake_fraction(ji,jk)) &
          *0.5_wp*c_d_v*(state_new_used%rho(ji,jk,n_layers,n_condensed_constituents+1) &
          ...
          *(t_gas_lowest_layer_new - diag%sst(ji,jk)))/diag%scalar_flux_resistance(ji,jk)
          ...
          -grid%area_z(ji,jk,n_levels)*(diag%power_flux_density_sens_soil(ji,jk)+diag%power_flux_density_sens_sea(ji,jk)) &
    ...
    do ji=1,ny
      do jk=1,nx
        ...
        if (soil_switch==1) then
          ...
          ! sensible heat flux
          + (diag%power_flux_density_sens_soil(ji,jk) &
          ! latent heat flux
          + diag%power_flux_density_lat_lake(ji,jk) &
