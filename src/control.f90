...
program control
  ...
  allocate(grid%geo_scal(nlins,ncols,nlays))
  allocate(grid%dx(nlins,ncols+1,nlays))
  allocate(grid%dy(nlins+1,ncols,nlays))
  allocate(grid%dz(nlins,ncols,nlays+1))
  allocate(grid%geo_u(nlins,ncols+1,nlays))
  allocate(grid%geo_v(nlins+1,ncols,nlays))
  allocate(grid%geo_w(nlins,ncols,nlays+1))
  ...
  allocate(grid%geo_area_dual_z(nlins+1,ncols+1,nlays))
  ...
  allocate(grid%soil_interface(nsoillays+1))
  allocate(grid%soil_center(nsoillays))
  ...
  deallocate(grid%geo_scal)
  deallocate(grid%dy)
  deallocate(grid%dx)
  deallocate(grid%dz)
  deallocate(grid%geo_u)
  deallocate(grid%geo_v)
  deallocate(grid%geo_w)
  ...
  deallocate(grid%geo_area_dual_z)
  ...
  deallocate(grid%soil_interface)
  deallocate(grid%soil_center)
