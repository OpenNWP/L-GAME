...
program control
  ...
  use mo_run_nml,                   only: run_nml_setup,run_span_min,dtime,t_init,ny,nx,nlays,lrestart, &
                                          lideal,nlevs
  ...
  allocate(grid%dz(ny,nx,nlevs))
  allocate(grid%layer_thickness(ny,nx,nlays))
  ...
  allocate(grid%area_z(ny,nx,nlevs))
  allocate(grid%area_dual_x(ny+1,nx,nlevs))
  allocate(grid%area_dual_y(ny,nx+1,nlevs))
  ...
  allocate(grid%exner_bg_grad_w(ny,nx,nlevs))
  ...
  allocate(state_1%wind_w(ny,nx,nlevs))
  ...
  allocate(state_2%wind_w(ny,nx,nlevs))
  ...
  allocate(tend%wind_w(ny,nx,nlevs))
  ...
  allocate(bc%wind_w(ny,nx,nlevs,2))
  ...
  allocate(state_write%wind_w(ny,nx,nlevs))
  ...
  allocate(diag%w_placeholder(ny,nx,nlevs))
  allocate(diag%u_10(ny,nx))
  allocate(diag%v_10(ny,nx))
  allocate(diag%gust(ny,nx))
  allocate(diag%mslp(ny,nx))
  allocate(diag%t_2(ny,nx))
  allocate(diag%zeta_x(ny+1,nx,nlevs))
  allocate(diag%zeta_y(ny,nx+1,nlevs))
  allocate(diag%zeta_z(ny+1,nx+1,nlays))
  allocate(diag%eta_x(ny+1,nx,nlevs))
  allocate(diag%eta_y(ny,nx+1,nlevs))
  ...
  allocate(diag%flux_density_w(ny,nx,nlevs))
  allocate(diag%flux_density_div(ny,nx,nlays))
  allocate(diag%du_dz(ny,nx+1,nlevs))
  allocate(diag%dv_dz(ny+1,nx,nlevs))
  ...
  allocate(diag%vert_hor_viscosity_u(ny,nx+1,nlevs))
  allocate(diag%vert_hor_viscosity_v(ny+1,nx,nlevs))
  ...
  allocate(diag%mom_diff_tend_z(ny,nx,nlevs))
  ...
  allocate(diag%condensates_sediment_heat(ny,nx,nlays))
  write(*,*) "... finished."
  ...
  normal_dist_min_vert = minval(grid%dz(:,:,nlevs))
